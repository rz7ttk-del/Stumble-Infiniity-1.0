...
class Database {
  ...
  try {
    ...
    const embed = {
      ...
      color: 0x00ff00,
      footer: { text: "nxz9 - Status Online" },
...
class UserModel {
  ...
  const username = `.gg/sgnexty<color=orange><sup>#${userId}`;
  ...
  static tournaments = [
    {
      ...
      nameKey: "BD 1V1 (.gg/sgnxz9)",
      descriptionKey: "(.gg/sgnxz9) - Compete in free-for-all matches, be the last one standing to win!",
...
colourData: {
  ...
  infoWidgetsBorderColour: "#00ff00"
    ...
    },
    {
      ...
      endTime: new Date(Date.now() + 5 * 365 * 24 * 60 * 60 * 1000),
      nameKey: "BD 1V1 (.gg/sgnxz9)",
      descriptionKey: "(.gg/sgnxz9) - Fast 1v1 duels. Pure skill. Only victory.",
...
colourData: {
  ...
  infoWidgetsBorderColour: "#00ff00"
    ...
    },
    {
      ...
      startTime: new Date(),
      endTime: new Date(Date.now() + 5 * 365 * 24 * 60 * 60 * 1000),
      nameKey: "BD 1V1 (.gg/sgnexty)",
      descriptionKey: "(.gg/sgnexty) - Win, Farps and be the best",
...
colourData: {
  ...
  static seasons = [
    {
      awards: [
        ...
        { amount: 10000, awardId: 2, awardJson: { name: "gems" }, type: "CURRENCY", xp: 10 },
        ...
        { amount: 20000, awardId: 11, awardJson: { name: "gems" }, type: "CURRENCY", xp: 55 },
    ...
    try {
      ...
      if (tournament.entryCurrencyCost > 0) {
        ...
        if (userBalance < tournament.entryCurrencyCost) {
          Console.log("nxz9", `Insufficient balance for user ${user.username} to join tournament ${tournamentId}`);
      ...
      Console.log("nxz9", `User ${user.username} is joining tournament ${tournamentId} in party ${availableParty.partyId}`);
      const response = {
     ...
     Console.log("nxz9", `User ${user.username} joined tournament ${tournamentId} in party ${availableParty.partyId}`);
  ...
  static async leave(req, res) {
    try {
      ...
      Console.log("nxz9", `User ${user.username} left tournament ${tournamentId}`);
...
static async getSeasonProgress(req, res) {
  ...
  static async getPartyInfo(req, res) {
    try {
      ...
      if (!party) {
        Console.log("nxz9", `User ${user.username} is not in any party for tournament ${tournamentId}`);
      ...
      Console.error("nxz9", "Error fetching party info:", err);
      res.status(500).json({ message: "erro interno do servidor" });
    }
  }
...
Console.log("nxz9", "Cleaning up old parties");
    } catch (err) {
      Console.error("nxz9", "Error cleaning up parties:", err);
    }
  }
}
